if global.EnemyGroup > 4 {
	sprite_index = GasterBlaster_Black
}
if !Spawned {
	if (global.EnemyGroup == 3 || instance_exists(obj_GasterHands_Transition)) && !nos
		audio_play_sound(corruptblaster_ready, 20, false);
...
if (Timer > 0) {
	...
	if (Interval == 0) {
		...
		if (global.EnemyGroup == 3 || instance_exists(obj_GasterHands_Transition)) && !nos
			audio_play_sound(corruptblaster_fire, 20, false);
		Beam = instance_create_depth(x, y, depth + 1, obj_GasterBlasterBeam);
		Beam.NoDamage = NoDamage;
		if global.EnemyGroup > 4 {
			Beam.sprite_index = GasterBlaster_Beam_Black
		}
